      ...
      var i, support, Expr, getText, isXML, tokenize, compile, select,
          ...
          funescape =
            ...
            return nonHex ?
                ...
                // Replace a hexadecimal escape sequence with the encoded
                // Unicode code point Support: IE <=11+ For values outside the
                // Basic Multilingual Plane (BMP), manually construct a
                // surrogate pair
                high < 0 ?
    ...
    jQuery.each(tuples, function(i, tuple) {
      ...
      deferred[tuple[0]] = function() {
        deferred[tuple[0] + 'With'](
            this === deferred ? undefined : this, arguments);
        return this;
      };
...
jQuery.Deferred.exceptionHook = function(error, stack) {
  ...
  if (window.console && window.console.warn && error &&
    ...
    window.console.warn(
        'jQuery.Deferred exception: ' + error.message, error.stack, stack);
  }
};
...
var isHiddenWithinTree = function(elem, el) {
  ...
  return elem.style.display === 'none' ||
      ...
      // Disconnected elements can have computed display: none, so first confirm
      // that elem is in the document.
      isAttached(elem) &&
      ...
      if ((event.isTrigger & 1) && this[type]) {
        ...
        if (!saved.length) {
          ...
          if (saved !== result) {
            ...
            // that handler calls `.remove()` on the element, the data is
          ...
          if (rscriptType.test(node.type || '') &&
            ...
            if (node.src && (node.type || '').toLowerCase() !== 'module') {
              ...
              if (jQuery._evalUrl && !node.noModule) {
                jQuery._evalUrl(
                    node.src, {nonce: node.nonce || node.getAttribute('nonce')},
                    doc);
              }
            } else {
  ...
  if (!isBorderBox && computedVal >= 0) {
    ...
    delta += Math.max(
                 ...
                 Math.ceil(
                     ...
                     // If offsetWidth/offsetHeight is unknown, then we can't
                     // determine content-box scroll gutter Use an explicit zero
                     // to avoid NaN (gh-3964)
                     )) ||
        0;
  }
...
function getWidthOrHeight(elem, dimension, extra) {
  ...
  return (val +
          boxModelAdjustment(
              elem, dimension, extra || (isBorderBox ? 'border' : 'content'),
      ...
      var matches, styles = getStyles(elem),
          ...
          scrollboxSizeBuggy =
              !support.scrollboxSize() && styles.position === 'absolute',
                   ...
                   // To avoid forcing a reflow, only fetch boxSizing if we need
                   // it (gh-3991)
...
jQuery.extend(jQuery.event, {
  ...
  simulate: function(type, elem, event) {
    var e = jQuery.extend(
        new jQuery.Event(), event, {type: type, isSimulated: true});
